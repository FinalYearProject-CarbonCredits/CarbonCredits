...
def my_issuances(user: User = Depends(require_landowner), db: Session = Depends(get_db)):
    """All issuance/verification records across this landowner's listings."""
    listing_ids = [l.id for l in db.query(LandListing).filter(LandListing.owner_user_id == user.id).all()]
    if not listing_ids:
        return {"count": 0, "issuances": []}
    recs = (
        db.query(CreditIssuance)
        .filter(CreditIssuance.listing_id.in_(listing_ids))
...
@router.post("/listings/{listing_id}/verification/submit", status_code=201)
def submit_for_verification(
    ...
    """
    Submit a published listing into the verification & issuance pipeline.
    ...
    if not parcel or parcel.verification_status != "VERIFIED":
        raise HTTPException(
            status_code=403,
            detail="Land must be admin-verified before submitting for registry issuance",
        )
    ...
    if not kyc or kyc.status != "VERIFIED":
        raise HTTPException(
            status_code=403,
            detail="KYC must be verified before submitting for registry issuance",
        )

    existing = db.query(CreditIssuance).filter(
        CreditIssuance.listing_id == listing_id,
        CreditIssuance.status.notin_(["REJECTED"]),
    ).first()
    if existing:
        raise HTTPException(
            status_code=400,
            detail="An issuance request is already in progress for this listing",
        )
    ...
    rec = CreditIssuance(
        ...
        preliminary_annual_tco2e=listing.estimated_annual_credits_tco2,
        evidence_notes=body.evidence_notes,
    )
    advance_status(rec, "SUBMITTED")
    db.add(rec)
    db.commit()
    db.refresh(rec)
